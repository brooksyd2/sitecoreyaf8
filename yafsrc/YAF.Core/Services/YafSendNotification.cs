    ...
    public class YafSendNotification : ISendNotification, IHaveServiceLocator
    {
        ...
        public void ToModeratorsThatMessageNeedsApproval(int forumId, int newMessageId, bool isSpamMessage)
        {
            ...
            foreach (var userName in moderatorUserNames.Distinct())
            {
                ...
                var SitecoreMembershipUser = UserMembershipHelper.GetUser(userName);
                var userId = UserMembershipHelper.GetUserIDFromProviderUserKey(SitecoreMembershipUser.ProviderUserKey);
                ...
                notifyModerators.SendEmail(
                    new MailAddress(SitecoreMembershipUser.Email, SitecoreMembershipUser.UserName),
                    subject,
                    true);
            }
        }
        ...
        public void ToWatchingUsers(int newMessageId)
        {
            ...
            foreach (var message in LegacyDb.MessageList(newMessageId))
            {
                ...
                foreach (var user in usersWithAll.Where(x => x.UserID != userId))
                {
                    ...
                    var SitecoreMembershipUser = UserMembershipHelper.GetUser(user.ProviderUserKey);

                    if (SitecoreMembershipUser == null || !SitecoreMembershipUser.Email.IsSet())
                    {
                        continue;
                    }
